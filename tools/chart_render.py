...
<style>
  ...
  .key-dot{display:inline-block;width:14px;height:0;border-top:2px dashed var(--c,#9aa0ad);
    margin:0 4px 0 10px;vertical-align:middle}
  .label .kname{color:#9aa0ad}
...
<div class="label" data-label-for="macd">MACD (12,26,9)<span class="key-dot" style="--c:#5b8def"></span><span class="kname">MACD line</span><span class="key-dot" style="--c:#e0a73e"></span><span class="kname">signal</span></div>
<div class="pane" id="macd"><div id="mask-left-macd" class="dim-mask"></div><div id="mask-right-macd" class="dim-mask"></div></div>
<div class="label" data-label-for="stoch">Stochastic (14,3)<span class="key-dot" style="--c:#5b8def"></span><span class="kname">%K</span><span class="key-dot" style="--c:#e0a73e"></span><span class="kname">%D</span></div>
...
const sma50S  = price.addLineSeries({color:'#e0a73e',lineWidth:2,priceLineStyle:3,priceLineColor:'rgba(224,167,62,0.4)'});
const sma200S = price.addLineSeries({color:'#f5d76e',lineWidth:2,priceLineStyle:3,priceLineColor:'rgba(245,215,110,0.4)'});

const rsiS = rsiC.addLineSeries({color:'#e0a73e',lineWidth:1,priceLineStyle:3,priceLineColor:'rgba(224,167,62,0.4)'});
rsiS.createPriceLine({price:70,color:'rgba(200,204,212,0.4)',lineStyle:3,title:''});
rsiS.createPriceLine({price:30,color:'rgba(200,204,212,0.4)',lineStyle:3,title:''});

// histogram lives on its OWN overlay scale so it never rescales with the MACD/signal
// lines — a fixed centred band, the way volume is pinned independently in the price pane
const macdHist = macdC.addHistogramSeries({priceScaleId:'mhist',priceLineVisible:false});
macdHist.priceScale().applyOptions({scaleMargins:{top:0.28,bottom:0.28}});
const macdLine = macdC.addLineSeries({color:'#5b8def',lineWidth:1,priceLineStyle:3,priceLineColor:'rgba(91,141,239,0.4)'});
const macdSig  = macdC.addLineSeries({color:'#e0a73e',lineWidth:1,priceLineStyle:3,priceLineColor:'rgba(224,167,62,0.4)'});

const kS = stoch.addLineSeries({color:'#5b8def',lineWidth:1,priceLineStyle:3,priceLineColor:'rgba(91,141,239,0.4)'});
const dS = stoch.addLineSeries({color:'#e0a73e',lineWidth:1,priceLineStyle:3,priceLineColor:'rgba(224,167,62,0.4)'});
kS.createPriceLine({price:80,color:'rgba(200,204,212,0.4)',lineStyle:3,title:''});
kS.createPriceLine({price:20,color:'rgba(200,204,212,0.4)',lineStyle:3,title:''});
...
function loadResolution(res){
  ...
  macdHist.setData(d.macd.hist.map(p =>
    (p.value == null) ? {time:p.time}
      : {time:p.time,value:p.value,color:p.value>=0?'rgba(38,166,154,0.4)':'rgba(239,83,80,0.4)'}
  ));
  ...
  clearSR();
  if(d.support != null){ srLines.push(candles.createPriceLine({price:d.support,color:'rgba(200,204,212,0.4)',
    lineWidth:1,lineStyle:3,axisLabelVisible:true,title:'S'})); }
  if(d.resistance != null){ srLines.push(candles.createPriceLine({price:d.resistance,color:'rgba(200,204,212,0.4)',
    lineWidth:1,lineStyle:3,axisLabelVisible:true,title:'R'})); }
...
// Default view: price + volume only. Everything else (BB, MA50, MA200, RSI, MACD,
// Stochastic) starts OFF — reuse the toggle handlers so pane layout/axis stay correct.
['bb','sma50','sma200','rsi','macd','stoch'].forEach(t => {
  const chip = document.querySelector('.tog[data-toggle="'+t+'"]');
  if(chip && !chip.classList.contains('off')) chip.click();
});
...
_LEGEND = (
    ...
    "<b>MACD</b> (blue line) crossing its signal (gold) = momentum shift; histogram = the gap. "
    "<b>Stochastic</b> %K (blue) / %D (gold); &gt;80 overbought / &lt;20 oversold. "
